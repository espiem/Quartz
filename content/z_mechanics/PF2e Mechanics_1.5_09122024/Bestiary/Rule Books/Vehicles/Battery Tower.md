---
title: Battery Tower
obsidianUIMode: preview
noteType: pf2eVehicle
cssClasses: pf2e
tags:
  - pf2eVehicle

source: Pathfinder Guns &amp; Gears
aliases: "Actor.gDsbqpn6dwTMasQj" 
level: 16
license: OGL
statblock: inline
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Guns & Gears"
name: "Battery Tower"
level: "Vehicle 16"
rare_03: [[Rare]]

size: "Gargantuan"
modifier: 
perception:
  - name: ""
  - name: "Price"
    desc: "30000 gp\n* * *"
  - name: ""
    desc: "The battery tower uses both pushed propulsion and turned propulsion to turn a capstan. Turned propulsion uses the same rules as rowed propulsion. Pushed propulsion uses the same rules as pulled.\n* * *"
abilities_top:
  - name: ""
  - name: "Space"
    desc: "50 feet long, 50 feet wide, 90 feet high"
  - name: "Crew"
    desc: "1 pilot, 150 turners, 2200 pushers; **Passengers** 204"
  - name: "Piloting Check"
    desc: "Diplomacy (DC 37), Intimidation (DC 37), or Warfare Lore (DC 35)"
ac: 34
armorclass:
  - name: AC
    desc: "34; __Fort__ +28"
hp: 150
health:
  - name: ""
  - name: "Hardness"
    desc: "25; **HP** 150 (BT 75); __Immunities__  object immunities"
speed: "10 feet (pushed, turned)"
abilities_mid:
  - name: ""
attacks:
  - name: ""
  - name: "Collision"
    desc: "8d10 (DC 32)"

  - name: "Sluggish"
    desc: " This vehicle must move twice its length for each 90-degree turn it makes."

  - name: "Tiered"
    desc: " A battery tower has five levels. The crew operate at the bottom tier, turning a giant capstan, and up to four passengers can fit on that floor, enough for a full crew for a bombard. The remaining levels carry 50 passengers each. The tiers are specifically meant to be protective, and give those within the battery tower greater cover (or standard cover if the battery tower is [[Conditions/Broken|Broken]]). A massive ladder runs diagonally through the center of the battery tower, from the front of the top tier to the rear of the bottom tier. The ladder is wide enough for two crew or passengers to climb simultaneously."

  - name: "Weaponry"
    desc: " A battery tower is usually loaded with black powder siege weapons for heavy assaults, with bombards on the bottom two floors and cannons on the other floors. Their Price is included in that of the battery tower. When building a battery tower, you can choose to replace any of these with other siege weapons of a lower Price. If you want a more expensive siege weapon, you must Craft it separately. Passengers of the battery tower can crew these siege weapons."



sourcebook: "_Pathfinder Guns & Gears_"
```

```encounter-table
name: Battery Tower
creatures:
  - 1: Battery Tower
```

